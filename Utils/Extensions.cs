...
namespace QuestHome
{
    internal static class Extensions
    {
        ...
        public static bool HasKey(this IniData config, string section, string key)
        {
            if (!config.Sections.ContainsSection(section)) return false;
            if (!config[section].ContainsKey(key)) return false;
            return true;
        }
    }
